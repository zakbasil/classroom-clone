...
import {
  ...
  getPersonalMaterialTemplates,
  getMaterialTemplate,
  createMaterialTemplate,
  updateMaterialTemplate,
  deleteMaterialTemplate,
  publishMaterialTemplate,
  unpublishMaterialTemplate,
  requestApprovalMaterialTemplate,
...
export function MaterialTemplatesTab() {
  const { isAdmin } = useAuth();
  const [approvedTemplates, setApprovedTemplates] = useState<MaterialTemplate[]>([]);
  const [personalTemplates, setPersonalTemplates] = useState<MaterialTemplate[]>([]);
  const [allPersonalTemplates, setAllPersonalTemplates] = useState<MaterialTemplate[]>([]);
  ...
  const handleEdit = async (template: MaterialTemplate) => {
    try {
      const fullTemplate = await getMaterialTemplate(template.id);
      setEditingTemplate(fullTemplate);
      setTitle(fullTemplate.title);
      setDescription(fullTemplate.description || '');
      setTopic(fullTemplate.topic || '');
      setFileUrl(fullTemplate.fileUrl || '');
      setLinkUrl(fullTemplate.linkUrl || '');
    } catch (error) {
  ...
  const handleUpdate = async () => {
    ...
    setIsCreating(true);
    try {
      await updateMaterialTemplate(editingTemplate.id, {
        title,
        description: description || undefined,
        topic: topic || undefined,
        fileUrl: fileUrl || undefined,
  ...
  const handleRequestApproval = async (templateId: string) => {
    try {
      await requestApprovalMaterialTemplate(templateId);
  ...
  const handlePublish = async (templateId: string) => {
    try {
      await publishMaterialTemplate(templateId);
  ...
  const handleUnpublish = async (templateId: string) => {
    try {
      await unpublishMaterialTemplate(templateId);
  ...
  const TemplateCard = ({ template, isPersonal }: { template: MaterialTemplate; isPersonal: boolean }) => (
    <Card key={template.id}>
      ...
      <CardContent>
        ...
        {(template.fileUrl || template.linkUrl) && (
          <div className="text-xs text-muted-foreground">
            {template.fileUrl && <div>File: {template.fileUrl}</div>}
            {template.linkUrl && <div>Link: {template.linkUrl}</div>}
          </div>
        )}
      </CardContent>
    </Card>
  ...
  return (
    <div className="space-y-6">
      <div className="flex items-center justify-between">
        <div>
          <h2 className="text-xl font-semibold">Material Templates</h2>
        ...
        <Dialog open={isCreateOpen} onOpenChange={setIsCreateOpen}>
          ...
          <DialogContent>
            <DialogHeader>
              <DialogTitle>Create Material Template</DialogTitle>
              <DialogDescription>
                Create a reusable material template
              </DialogDescription>
            </DialogHeader>
            <div className="space-y-4">
              <div className="space-y-2">
                ...
                <Input
                  ...
                  placeholder="Material Template Title"
                />
              </div>
              <div className="space-y-2">
      ...
      <Dialog open={!!editingTemplate} onOpenChange={(open) => !open && setEditingTemplate(null)}>
        <DialogContent>
          <DialogHeader>
            <DialogTitle>Edit Material Template</DialogTitle>
            <DialogDescription>
              Update your material template
            </DialogDescription>
          </DialogHeader>
          <div className="space-y-4">
            <div className="space-y-2">
